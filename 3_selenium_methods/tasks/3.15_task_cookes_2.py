...
with webdriver.Chrome() as driver:
    driver.get(link)
    skillset_length, age, cookie_value = 0, 100, str()
    for cookie in cookies:
        driver.add_cookie(cookie)
        driver.refresh()
        skillsets = len(driver.find_elements(By.CSS_SELECTOR, 'skillsList > li'))
        if skillsets > skillset_length or (skillsets == skillset_length and age > int(driver.find_element(By.ID, 'age').text.split()[1])):
            skillset_length = skillsets
            age = int(driver.find_element(By.ID, 'age').text.split()[1])
